...
function clipList(items: string[], max = 5) {
...
export function normalizeAnalysis(value: ReportAnalysis): ReportAnalysis {
  return {
    ...
    keyFindings: clipList(value.keyFindings),
    risks: clipList(value.risks),
    nextActions: clipList(value.nextActions),
    missingInformation: clipList(value.missingInformation),
    excludedDetails: clipList(value.excludedDetails, 8),
  };
}
